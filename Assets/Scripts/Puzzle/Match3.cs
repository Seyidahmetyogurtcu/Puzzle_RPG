...
namespace Puzzle_RPG
{
    ...
    public class Match3 : MonoBehaviour
    {
        ...
        public void SetLevelSO()
        {
            ...
            score = 0;
            moveCount = this.levelSO.moveAmount;

            OnLevelSet?.Invoke(this, new OnLevelSetEventArgs
            {
                levelSO = this.levelSO,
                grid = grid
            });
        }
        ...
        public void MoveUpGemsIntoEmptyPositions()
        {
            for (int x = 0; x < gridWidth; x++)
            {
                for (int y = gridHeight-1; y >=0; y--)//scan from up to down
                {
                    GemGridPosition gemGridPosition = grid.GetGridObject(x, y);
